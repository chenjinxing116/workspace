var loadwnd = null;
$(function() {
	$('#timeStart').val(new Date().Format("yyyy-MM-dd hh:mm:ss"));
	$('#timeEnd').val(new Date().Format("yyyy-MM-dd hh:mm:ss"));
...
function onSearch() {
	...
	var allocations = $("#selectFPQK").val(); //分配情况
	...
	switch($('#selectSJFW').val()) {
		...
		case "2":
			...
			break;
		case "3":
			beginTime = getTimeByDateStr($('#timeStart').val());
			endTime = getTimeByDateStr($('#timeEnd').val());
			break;
	}
